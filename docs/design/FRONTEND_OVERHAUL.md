...
## Phase 9 — Polish & acceptance — GREEN (R9 complete; redesign R0–R9 complete)

Ten steps, spread across three sessions (see SESSION_LOG.md Sessions 9–11 for the full
per-step detail — this report consolidates the phase, it doesn't replace those entries).

Scope delivered:
- **Step 1 — Stemma camera-fit bug** (logged at R6/R7): cytoscape-cola fires
  `'layoutstop'` twice on graphs small enough to hit its own convergence threshold early;
  `StemmaCanvas` treated the first (premature) stop as final and never refit on the real
  settle. Fixed with an elapsed-time gate plus a guard for the separate case of
  `runPhysics()` superseding a still-running layout. Three regression tests, each proven
  non-vacuous by reverting the fix and confirming they fail.
- **Step 2 — label legibility** (§16): a hub principal's focus fit was framing all of its
  near-nodes, shrinking zoom below the 13px floor on synthetic-100. Fixed with
  `FIT_FOCUS_BUDGET` — the camera fits the focus node plus its 12 spatially nearest ties
  (every tie still draws normally; only the camera framing is capped). An
  importance-ranked version was tried first and measured to barely move the zoom at all
  — kept as a documented dead end.
- **Step 3-4 — global keyboard map + screen-reader mirror** (§8.5, §11):
  `codex/shortcuts/` (`g d`/`g w`/`g c` chords, `?` shortcut sheet, real focus trap) and
  an off-screen `aria-live` mirror of the Stemma's focused neighbourhood, built only from
  the same fenced view model the canvas draws from.
- **Step 5 — responsive breakpoints** (§11): 1024–1279 collapses each screen's right
  panel (Chronicle's own: a bottom sheet) into a closed-by-default drawer; <1024 adds the
  left rail as a second drawer. A real CSS cascade-order bug (responsive `@media` blocks
  declared before the base rules they were meant to override) meant the first-pass
  drawers silently never activated — found via computed-style inspection, not guessed;
  fixed by moving the blocks to the end of each stylesheet.
- **User-reported bugs, fixed the same session as step 5** (not part of the original 10
  steps, but found and closed before continuing): hidden-but-functional scrollbars
  globally; a small-cast Stemma focus fit that could clip a dimmed context node off the
  canvas edge (fixed by fitting the whole connected web below `FIT_ALL_WHEN_SMALL`); a
  reported font-integration issue that a full app-wide font/network/lint audit could not
  reproduce (documented ASSERTED-clean, not claimed fixed).
- **Step 6 — reduced-motion audit** (§4.5/§6.7): three of the four spec requirements
  (physics finite, no thread drawing, reveal collapses to a 200ms fade) were already
  correct, built incidentally at R5-R7 — verified, not re-built, by a new
  `tests/reduced-motion.spec.ts` (4 tests) that measures each one directly rather than
  assuming from the code. The fourth — the §6.7 loading glyph (a 3-dot ink→dim→faint
  indicator, static under reduced motion) — turned out not to exist at all: every
  `StateCard` "Loading" call site passed `body=""`, silently dropping both the required
  sentence and the dots. New `codex/states/LoadingDots.tsx` + `StateCard`'s `loading`
  prop fixed all three call sites. `scripts/shoot.mjs` gained a `--reducedMotion=reduce`
  flag for visual verification.
- **Step 7 — favicon/OG** (§14): `public/favicon.svg` (a wax-seal disc + bold-serif "S" —
  a generic serif substitutes for Pirata One, which a standalone favicon resource can't
  load) and a 1200×630 `public/og-image.png`, generated by rendering the real
  `.mural`/`.vignette` classes and landing copy through Playwright rather than
  hand-drawing an approximation. Real bug found and fixed while building the favicon: its
  own XML comment contained the substring `--bg`, and XML comments cannot contain `--`
  anywhere — every browser silently failed to parse the file. `index.html` gained the
  icon link, a meta description, and og:/twitter: tags.
- **Step 8 — delete `#/_legacy` and all legacy code, with grep proof:** before deleting
  anything, found a real conflict and took it to you rather than guessing — the
  spec-required "Add a novel" link (§6.1) had its only working implementation inside the
  legacy app (R8's deliberate choice not to build new ingestion UI), so a literal
  wholesale deletion would have silently killed the app's only ingestion path. You chose
  to port the Composer into the Codex UI as a real route (`#/add`) rather than lose the
  feature. Deleted whole: `App.tsx`, `GraphView.tsx`, `Library.tsx`, the old
  `Composer.tsx`, `LegacyRoute.tsx`, `styles.css`. Trimmed (not deleted) the files with a
  real, still-needed half: `ontology.ts` (kept `DEMO_SLUG`/`IDENTITY_RELATIONS`/
  `RELATION_LABELS`, dropped the old Cytoscape-only color constants), `api.ts` (dropped
  `fetchGraph`/`deleteWork`, both legacy-only), `lint-design.mjs` (dropped the
  `LEGACY_FILES` exemption list and the whole styles.css-collision rule 4 — both existed
  only for files that no longer exist), `shots.config.mjs` (dropped every
  `#/_legacy`-targeting shot). `grep -rln "_legacy"` across `frontend/{src,scripts,tests}`
  after all edits returns exactly 4 files, all historical comments, zero live code paths.
- **Step 9 — §16 acceptance checklist, line by line:** all 11 lines MEASURED and checked
  off in `DESIGN_SPEC.md` §16 itself; nine were already covered by existing passing
  suites (cited per line in SESSION_LOG Session 11), two needed a fresh look this session
  (the reduced-motion line, and a live screenshot confirming no mural line bleeds through
  the Stemma canvas mask — previously only structurally asserted, never actually looked
  at).
- **Step 10 — final full suite + full shoot, closing the phase:** every suite re-run
  individually (green — see SESSION_LOG Session 11 for the full tally), and every
  existing shot config (phases 1–9, 126 shots total across both required viewports)
  re-run fresh. Found and fixed one real, non-flaky failure: phase 3's `toast-forward`
  shot predated R6 and still expected a plain forward toast on a 3→4 move that R6 made
  into a reveal — fixed with the same `stripReveals`-style route interception
  `fence.spec.ts` already uses for the identical problem.

Spec sections covered: §4.5 (reduced motion), §6.7 (the loading glyph), §8.5 (keyboard
map), §11 (screen-reader mirror, responsive breakpoints), §14 (favicon/OG), §16 (the full
acceptance checklist) — the whole polish/acceptance phase, plus the wholesale legacy
deletion R2–R8 had been deferring to this phase throughout.

Deleted old code: `App.tsx`, `GraphView.tsx`, `Library.tsx`, the old `Composer.tsx`,
`LegacyRoute.tsx`, `styles.css` — all whole-file, listed with the grep proof above. Ported
(not deleted): the Composer's ingest logic, now `codex/Compose/`.

Backend deps hit: none new. The ported Composer calls the same four routes
(`ingestWork`/`previewChapters`/`fetchStatus`/`appendChapters`) it always did, unchanged
per R3's backend freeze.

Deviations kept (with reason):
- The favicon's "S" is a generic bold serif, not literally Pirata One (see step 7 above)
  — a standalone SVG resource has no access to the page's `@font-face` rules, and
  embedding the whole variable font as base64 to label a 16px icon isn't a reasonable
  trade.
- `Composer`'s `mode="append"` (add chapters to an existing novel) is kept fully working
  in the ported component but has no reachable entry point in the redesign — the old
  per-work "add chapters" trigger lived in the retired Library screen, and nothing in
  DESIGN_SPEC specs a replacement for it. Documented in the component's own header
  comment rather than silently dropped; a future phase can wire a trigger for it if
  wanted.

Issues found and fixed this phase (all MEASURED, none left BROKEN): the Stemma
double-`'layoutstop'` camera-fit bug (step 1); the CSS cascade-order bug that silently
disabled the R9 responsive drawers (step 5); the missing loading-dots glyph (step 6); the
XML-comment `--bg` favicon parse failure (step 7); the stale pre-R6 `toast-forward` shot
(step 10). Three items reported by you mid-session and resolved the same session:
scrollbars, small-cast node clipping, and an investigated-but-unreproduced font concern
(documented ASSERTED-clean).

Interview-defence note: this phase's two hardest bugs (the double-layoutstop camera fit,
and the CSS cascade order silently disabling the responsive drawers) were both root-caused
with direct instrumentation — a timestamped event log for the first, computed-style
inspection for the second — rather than guessed from screenshots or trial-and-error CSS
changes. The legacy-deletion conflict (step 8) is the phase's other defining moment: the
plan said "delete `#/_legacy` wholesale," and the mechanical thing to do was follow it: but
reading what actually depended on that route first surfaced a real, spec-required feature
that would have silently broken. Taking that to you rather than either guessing an answer
or deleting-and-hoping is the same discipline the rest of this project has tried to hold
to throughout — no black boxes, nothing shipped on an assumption that could have been
checked.

MEASURED (full suite, R9 step 10): backend `pytest` 124 passed / 6 skipped (unchanged
throughout the entire redesign). `typecheck` clean. `build` clean, 791KB (down from 812KB
pre-deletion — the removed legacy chunk was bigger than what replaced it). `lint:design`
83 files clean. `test:unit` 63/63. `test:fence` 29/29 (+2 fixme, F6/F7 still
config-absent — D6 arc config was never wired up, out of scope for this redesign).
`test:dossier` 6/6. `test:stemma` 16/16. `test:reveal` 11/11. `test:reveal-choreography`
3/3. `test:chronicle` 8/8. `test:landing` 9/9. `test:shortcuts` 5/5. `test:style` 5/5.
`test:geometry` 22/22. `test:reduced-motion` 4/4 (new this phase). Full shoot: 126 shots,
both viewports, zero console errors, spot-checked by eye.

Commit: (see SESSION_LOG.md Sessions 9-11) pushed: yes.

Next: none within the redesign track. **R0–R9 are all complete.** Per R1, `main` stays
frozen and untouched until you decide to merge `redesign/codex` — that decision, and its
timing, is yours.
