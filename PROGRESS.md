...
## Current phase
**Between phases — Q3 re-measure DONE (Measurement run 2); Phase 8 gate cleared on the over-merge axis.** The entailment gap was re-measured at scale on LotM ch1–5: **real-but-irrelevant over-merge rate = 0** across two runs → the parked NLI verifier stays **PARKED with evidence**, and the over-merge/spoiler axis does **not** block Phase 8 (caveats below). Last shipped product phase = **7c-fix** (relation-family-aware citation: `TRANSMIGRATED_INTO`/`REINCARNATION` accept a real in-range SINGLE-ANCHORED clue; the others keep the co-naming prompt; fabrication floor unchanged). **Next (decided): NOT NLI** — candidate next builds are Phase 8 (frontend) and/or the small quality fixes (relation-subtype discipline, the malformed-relation hardening), each subject to the universality guard. NO build this session. See In-progress / next + Known issues + Decisions/working rules.
...
- **DECIDED next-session order (updated by the 7c-fix):**
  ...
  4. **NEXT (candidates, NOT yet started — each subject to the universality guard):**
     - **Phase 8 (Frontend)** — now unblocked on the over-merge/spoiler axis (the gate it was waiting on). The biggest remaining value; reveal timing is over-merge-safe.
     - **Small quality fixes** (optional, can fold into/precede Phase 8): (b) **relation-subtype discipline** (model labels TRANSMIGRATED_INTO as REINCARNATION, etc.); the **malformed-`relation` robustness** hardening (see Known issues). Neither is an over-merge/spoiler risk.
     - **NLI/entailment verifier — DO NOT BUILD** until a range with real hidden-secret-identity reveals shows the gap actually biting (none observed in two runs).
  5. **Phase 7d (Tier-2 LLM cleanup), still PARKED:** LLM-assisted Tier-2 disambiguation (Hermes-language vs deity) + junk rejection. Its own boundary; must NOT disturb the 7a precision (P=1.00) number or the Tier-3 edges. **Distinct from the Tier-3 entailment gap.** **Phase 9** — packaging/CI/README.
