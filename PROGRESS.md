...
## Current phase
**Phase 7b — LLM go/no-go gate (+ graded-probe addendum).** Status: **GO, green & pushed**. Local LLM path proven on both devices via Ollama; chosen path wired behind `llm_enabled` (OFF by default). The graded addendum (below) hardened the easy single-clue probe into a 4-case discrimination test and **revised the 7c model recommendation to GPU `llama3.2:3b` primary** (was CPU `qwen2.5:7b`) — accuracy-equal to 7B on every graded case incl. the negative control, 6.5× faster. NO 7c inference code yet. Next: Phase 7c (Tier-3 identity inference) or Phase 8 (frontend).

## Done (Phase 7b ADDENDUM — graded identity probe; the model choice is now evidence-backed)
The 7b gate's single Wren==Caelum clue was too easy (all 4 configs solved it → the model pick rested on size reasoning, not accuracy). This addendum builds a **graded, reveal-windowed, 4-case probe** (`eval/llm_probe.py`, stdlib-only) and runs the FULL set on both finalists at their working configs. **No 7c pipeline, no identity edges written** — probe + verdict only; Ollama stopped at the end.
- **Cases (each fed only the chapters where its fact is knowable — reveal discipline):** C1 Wren==Caelum (ch1-2, gold MERGE — easy baseline); C2 Gray Sparrow==Lady Veris (ch1-3, gold MERGE — the alias); C3 transmigration/outside-soul (ch1-4, gold TRUE — subtlest); **C4 NEGATIVE CONTROL** (ch1-4, gold = NO false merge). C4 grades the full grouping and FAILS on any group mixing two distinct people (Veris≠Maela, Caelum≠Maela, Wren≠Dunmore via a canonical-identity map) — it punishes over-merging.
- **THE TABLE** (base prompt; identical result under the `--cite` "quote a clue per merge" improvement):

  | model | dev | C1 Wren=Caelum | C2 Sparrow=Veris | C3 Transmig | C4 NegControl | tok/s | ALL | merge-P |
  |---|---|---|---|---|---|---|---|---|
  | llama3.2:3b | GPU | **OK** | **X** | **OK** | **OK** | **53.8** | no | 1.00 |
  | qwen2.5:7b | CPU | **OK** | **X** | **OK** | **OK** | 8.0 | no | 1.00 |

  **Both models are accuracy-identical: 3/4, same pass/fail, both miss C2, both pass the negative control (zero false merges, merge-precision 1.00).** The `--cite` prompt improvement moved nothing.
- **Why C2 fails — and why it does NOT separate the models:** both *under-merge* — they latch onto the protagonist's identity and emit only `[Wren, Caelum]`, ignoring the explicitly-stated `[Sparrow, Veris]` in the same passage. A direct per-pair probe ("are the Gray Sparrow and Lady Veris the same person?") returns **`true` on BOTH models** → the alias is KNOWN; the miss is an *enumeration artifact* of the holistic "find all groups" framing, shared by both sizes. **7c design lesson: infer identity per-candidate-pair (propose pairs from co-occurring entities, ask the LLM to confirm), not by one-shot enumeration.**
- **VERDICT (evidence-backed, revises the 7b size-based guess): 7c primary = GPU `llama3.2:3b`; fallback = CPU `qwen2.5:7b`.** Justified by the table, not size: GPU-3B passes C1/C3/C4 and does NOT false-merge the negative control; it misses C2 — but so does 7B (no 7B accuracy advantage anywhere), and the `--cite` fix didn't help either. With accuracy tied across all four graded cases, the only discriminator that moved is **speed: 53.8 vs 8.0 tok/s (~6.5×)** → the faster model wins by default. CPU-7B stays a first-class fallback for VRAM-contended runs (GLiNER holding the card) / RAM-only machines, and as the conservative choice to **re-validate on a real novel in 7c** before locking (the CC0 sample is still modest; 7B's "safer on harder multi-step reasoning" edge is *unsupported* here, not disproven).
...
## In-progress / next
- **Phase 7b DONE — GO** (+ graded-probe addendum). The local LLM path works (Ollama, OpenAI-compatible, F: weights, OFF by default). **Recommendation for 7c is now GPU `llama3.2:3b` primary + CPU `qwen2.5:7b` fallback** (revised by the addendum table: accuracy-tied across 4 graded cases incl. the negative control, GPU-3B 6.5× faster). Decide the next step:
...
- **Phase 7b ADDENDUM (model pick = GPU `llama3.2:3b` primary, CPU `qwen2.5:7b` fallback — now EVIDENCE-backed):** a 4-case graded probe (`eval/llm_probe.py`: 2 alias positives, a transmigration case, and a negative control that fails on any false merge) run on both finalists shows them **accuracy-identical (3/4 each, both miss C2 Sparrow=Veris, both pass the negative control, merge-precision 1.00); the `--cite` improvement changed nothing.** C2's miss is a shared *under-merge/enumeration* artifact (a direct per-pair probe returns `true` on BOTH → both KNOW the alias), so it gives 7B no edge. With accuracy tied, **speed decides: 53.8 vs 8.0 tok/s (~6.5×) → flip primary to GPU-3B.** CPU-7B remains the fallback for VRAM-contended/RAM-only runs and the conservative re-validation choice on a real novel in 7c. **7c lesson banked: do identity inference per-candidate-pair, not one-shot "find all groups."**
