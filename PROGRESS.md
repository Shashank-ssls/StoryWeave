...
## Current phase
**Phase 8 — frontend COMPLETE + POLISHED, green & pushed, SCREENSHOT-VERIFIED.** Part A rebuilt the graph view; Part B added the library/ingest/search UI; this **focused-polish pass** shipped five specific fixes (below) — no redesign, everything working preserved. Phase 9 GATED on accurate output (the extraction-quality carry-forwards below are unchanged).
...
- **ROBUSTNESS finding (NEW, product bug — deferred, not fixed this measurement session):** `qwen2.5:7b` intermittently returns `relation` as a JSON **object** instead of a string; production `normalize_relation` assumes `str|None` and raises → `infer_identities` catches it and marks the **WHOLE run `degraded`** (one malformed model output zeroes the entire identity pass). Tiny hardening warranted (coerce/tolerate non-str in `normalize_relation`, or skip that one pair). See Known issues.
...
- **DECIDED next-session order (updated by the 7c-fix):**
  ...
  5. **NEXT (candidates — none started; see Deferred section for the full list):**
     ...
     - **Small quality fixes:** relation-subtype discipline; the malformed-`relation` robustness hardening.
     - **NLI/entailment verifier — DO NOT BUILD** until a contrasting-serial measurement shows the gap biting.
     - **Phase 7d (Tier-2 cleanup), PARKED** (must not regress 7a P=1.00); **Phase 9** (Docker/CI/README + hero GIF) — GATED on accurate output, GIF on CC0 only.
...
3. **`qwen2.5:7b` JSON-object `relation` bug** — intermittent object-instead-of-string makes `normalize_relation` raise → whole identity run marked `degraded`; tiny hardening (coerce/skip), do in a product session.
4. **Subtype imprecision** — TRANSMIGRATED_INTO vs REINCARNATION mislabel observed (both single-anchor so behavior was correct, the label was not); relation-subtype discipline (few-shot / normalizer).
5. **Phase 7d (Tier-2 disambiguation + junk rejection)** — still parked; must not regress 7a P=1.00.
6. **SAME_AS / REINCARNATION** — schema/prompt-ready, never exercised by a real case.
...
- **Malformed `relation` → whole-run degraded (NEW, product robustness, OPEN):** `qwen2.5:7b` intermittently returns `relation` as a JSON object; `normalize_relation` assumes `str|None` → raises → `infer_identities`' broad `except` marks the WHOLE work's identity pass `degraded` (one bad output zeroes the run). Tiny hardening: coerce/tolerate non-str in `normalize_relation` (or skip just that pair instead of failing the run). Not a spoiler/over-merge risk; do in a product session (deferred from the measurement run that found it).
