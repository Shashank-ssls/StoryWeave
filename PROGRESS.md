...
## In-progress / next
- **Phase 7c DONE.** Tier-3 identity inference shipped (citation-gated, reveal-respecting, OFF by default). **Identity primary = CPU `qwen2.5:7b`** (config default), GPU `llama3.2:3b` = speed fallback. Decide the next step:
  - **Phase 7d — Tier-2 LLM cleanup (PARKED here, its own boundary):** LLM-assisted Tier-2 disambiguation (Hermes-language vs deity) + junk rejection. Must NOT disturb the 7a precision (P=1.00) number or the Tier-3 edges. Separate phase on purpose.
  - **Phase 8 — Frontend**; **Phase 9 — packaging/CI/README**.
- **7c residual / next design inputs (recorded, NOT implemented):** (a) **citation RELEVANCE/entailment** — the existence gate verifies the quote is real, not that it entails the pair; GPU-3B can confabulate a real-but-irrelevant citation. A relevance check needs NLI/entailment (a stronger verifier than substring), or multi-sample agreement, not a both-names substring rule (which fails closed). (b) **relation-subtype discipline** — small models use SAME_AS/SECRET_IDENTITY/ALIAS interchangeably; a post-hoc type-normalizer or few-shot examples would firm this up. (c) **TRANSMIGRATED_INTO for the unnamed-soul case** — needs a single-entity property representation (or a synthetic "prior-world soul" node) rather than a two-named edge; revisit when a sample names both sides (LotM's Zhou Mingrui⇄Klein does).
- **Older 7b note (superseded by 7c):** the reveal-graded addendum recommended GPU `llama3.2:3b` for the *simple* per-pair probe (3B==7B, 6.8× faster); 7c's full task evidence moves the identity primary back to CPU `qwen2.5:7b`.
