...
## Current phase
**Phase 8 — frontend COMPLETE (Part A graph view + Part B library/ingest/search), green & pushed, SCREENSHOT-VERIFIED.** Part A rebuilt the graph view to a real bar after the blind first slice (root cause: Cytoscape's canvas doesn't parse `oklch()` → grey nodes). Part B adds the **multi-novel library, in-app ingest, and fenced search UI** — all server-fenced, screenshot-gated. Phase 9 GATED on accurate output (the extraction-quality carry-forwards below are unchanged).
