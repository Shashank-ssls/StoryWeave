...
## Current phase
**Phase 7c-fix — relation-family-aware citation rule (green & pushed).** Fixes the real-prose fail-closed the LotM ch1 run found: the citation requirement is now relation-family-aware — `TRANSMIGRATED_INTO`/`REINCARNATION` accept a real in-range SINGLE-ANCHORED clue (the reveal never co-names both entities), while `SAME_AS`/`ALIAS`/`SECRET_IDENTITY` keep the co-naming prompt; the fabrication floor is unchanged for all (no-citation-no-edge guard intact). Validated live on real LotM ch1. **Next (decided): re-measure the entailment gap (Q3) on a MULTI-CHAPTER LotM drop → only then (maybe) build the parked NLI verifier → 7d → Phase 8.** Phase 8 waits until reveal timing is trustworthy. See In-progress / next + Known issues.
...
- **DECIDED next-session order (updated by the 7c-fix):**
  ...
  3. **NEXT — re-measure the entailment gap (Q3) on a MULTI-CHAPTER LotM drop** with co-naming SECRET_IDENTITY/ALIAS reveals (the only cases the gap can bite). **BLOCKED on data:** only `data/raw/Lotm C-1.txt` is present (one chapter); `data/raw/lotm/` does not exist. Need ~10–15 contiguous chapters dropped locally (gitignored) before this can run. Only if Q3 bites at a real rate does the **parked NLI/entailment verifier** get built; otherwise lighter relation-subtype discipline. **Do not build NLI before this measurement.**
...
- **Tier-3 entailment gap (OPEN, blocks public reveal-timing claims):** the citation gate proves the cited clause is real, not that it entails the pair. Real-but-irrelevant citations can over-merge (GPU-3B observed on the sample). A Tier-3 SOUNDNESS gap in the identity showcase itself — **distinct from 7d (Tier-2 cleanup).** Must be resolved before any public GIF asserts trustworthy reveal timing.
  - **MEASURED (LotM ch1 run): the gap did NOT bite on real ch1 prose — 0 real-but-irrelevant over-merges** (it was masked: both models failed to quote the passage for the transmigration reveal → existence gate fail-closed on the TRUE reveal). The masking failure is **now FIXED** (see 7c-fix); **so the entailment gap is once again OBSERVABLE but still UNMEASURED at scale.** The heavy NLI fix stays **PARKED** (not justified by data yet). To justify/size it: re-measure Q3 (real-but-irrelevant over-merge rate) on a **multi-chapter LotM drop with co-naming SECRET_IDENTITY/ALIAS reveals** — the only cases where the gap can bite. **NOTE:** 7c-fix's single-anchor relax slightly widens the door for `TRANSMIGRATED_INTO`/`REINCARNATION` (a real one-name quote now validates even if only loosely relevant), so the re-measure matters a bit more for those two relations.
