...
## Current phase
**Phase 7c — Tier-3 IDENTITY inference (the showcase).** Status: **green & pushed.** Citation-gated, reveal-respecting identity edges (SAME_AS/ALIAS/SECRET_IDENTITY/REINCARNATION/TRANSMIGRATED_INTO) wired behind `llm_enabled` (OFF by default). Deterministic flip-at-gold + no-citation-no-edge regressions green in the light venv; the live model is reported, non-gating. **Identity primary refined to CPU `qwen2.5:7b`** (the full multi-step task re-separates the models — see below). NO Tier-2 cleanup (7d parked). **Next (decided): private LotM validation run → entailment-gate hardening → 7d → Phase 8** (Phase 8 waits until reveal timing is trustworthy). See In-progress / next + the open Tier-3 entailment gap in Known issues.
...
- **DECIDED next-session order (do not re-litigate):**
  1. **NEXT — private LotM validation run.** First contact of the 7c identity machinery with prose that was NOT hand-labeled. Measures three things: (a) does the citation gate hold, (b) does CPU-7B reveal timing survive a *real* reveal, (c) how badly does the entailment gap (see Known issues) actually bite. **LotM stays STRICTLY LOCAL** — gitignored `data/raw/`, never the public repo, never a GIF (copyrighted). **Size the entailment fix to what this run measures, not to theory.**
  2. **THEN — entailment-gate hardening,** scoped to the measured problem (not to the full theoretical fix list).
  3. **THEN — Phase 7d (Tier-2 LLM cleanup), still PARKED:** LLM-assisted Tier-2 disambiguation (Hermes-language vs deity) + junk rejection. Its own boundary; must NOT disturb the 7a precision (P=1.00) number or the Tier-3 edges. **Distinct from the Tier-3 entailment gap** (that is a Tier-3 soundness item, not Tier-2 — see Known issues).
  4. **Phase 8 (Frontend) does NOT start until reveal timing is trustworthy** (i.e. the entailment gap is resolved); **Phase 9** — packaging/CI/README.
...
## Known issues / TODOs
- **Tier-3 entailment gap (OPEN, blocks public reveal-timing claims):** the citation gate proves the cited clause is real, not that it entails the pair. Real-but-irrelevant citations can over-merge (GPU-3B observed). Fix candidates: NLI/entailment relevance check, relation-subtype discipline, unnamed-soul representation. Must be resolved before any public GIF asserts trustworthy reveal timing. **This is a Tier-3 SOUNDNESS gap in the identity showcase itself — distinct from 7d (Tier-2 cleanup).** Measure how hard it bites in the private LotM run first, then size the fix to that.
