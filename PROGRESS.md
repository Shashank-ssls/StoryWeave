...
## Current phase
**Phase 7b — LLM go/no-go gate (+ reveal-chapter-graded probe addendum).** Status: **GO, green & pushed**. Local LLM path proven on both devices via Ollama; chosen path wired behind `llm_enabled` (OFF by default). The addendum (below) replaced the easy single-clue probe with a **reveal-chapter-graded sweep** that measures *when* the model first flips to YES vs the chapter a careful reader could first conclude the identity — the metric 7c's `revealed_chapter` actually depends on. Verdict: **7c primary = GPU `llama3.2:3b`, fallback = CPU `qwen2.5:7b`**, plus a banked 7c design input (anchor the reveal flip to a cited confirming clause). NO 7c inference code yet. Next: Phase 7c (Tier-3 identity inference) or Phase 8 (frontend).
...
## In-progress / next
- **Phase 7b DONE — GO** (+ reveal-graded probe addendum). The local LLM path works (Ollama, OpenAI-compatible, F: weights, OFF by default). **Recommendation for 7c is now GPU `llama3.2:3b` primary + CPU `qwen2.5:7b` fallback** (reveal-chapter-graded sweep: with a cite-anchored prompt both hit all-EXACT + control PASS, GPU-3B ~6.8× faster; design input = gate the reveal flip behind a cited-clause confidence margin). Decide the next step:
