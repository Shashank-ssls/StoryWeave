...
## Current phase
**Phase 8 — frontend VERTICAL SLICE (green & pushed).** Vite/React/TS view over the fenced Phase-6 API: Cytoscape graph + 8-type legend, the **chapter slider driving the server-side fence**, and the **bloom** on reveal (Wren==Caelum SECRET_IDENTITY at ch2 is the signature moment). Fence stays server-side — the spoiler is ABSENT from the client payload at N=1, present at N=2 (proven at the payload level by `tests/test_demo_seed.py`, not pixels). Backend green (91 passed); frontend build green (`tsc -b && vite build`). **Deferred to later UI sessions: multi-novel library, in-app ingest, search UI** (see Deferred section). Phase 9 (Docker/CI/README + hero GIF) remains GATED on accurate output.
...
9. **Phase-8 follow-ups (deferred UI):** multi-novel library/home screen, in-app ingest (browser upload → local pipeline → SSE progress), semantic-search UI, evidence tooltips/zoom-dependent labels/type filters, and a Tier filter on `/graph`. The graph view + slider + bloom + legend shipped; the rest is later UI sessions.
10. **Frontend visual verification** — the slice's pixels were not machine-rendered this session (no browser tool); confirm the bloom visually on first `npm run dev`. Bundle is one ~593 kB Cytoscape chunk — code-split if Phase 9 cares about load time.
