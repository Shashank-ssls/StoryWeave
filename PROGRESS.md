...
## Current phase
**Phase 8 — frontend REBUILD, Part A (graph view) green & pushed, SCREENSHOT-VERIFIED.** The first slice shipped blind (no render) and was visually unacceptable — root cause: **Cytoscape's canvas does not parse `oklch()`, so every node rendered grey** (legend colours matched nothing), plus colliding labels and a static scatter. Part A rebuilds the visual/UX layer to a real bar, verified with a Playwright screenshot gate (Rule Zero). The fence/API/tests were correct and are preserved. **Part B remains: real novel library + in-app ingest + search UI** (the cover gives a clear entry; full library/ingest deferred). Phase 9 GATED on accurate output.
